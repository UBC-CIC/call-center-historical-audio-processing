...
![alt text](audio-processing-frontend-upload.png)
...
5) Select **Create new environment** under the **Select a backend environment** dropdown and leave the environment name 
   as default, then select your AWS Amplify service role in the following dropdown if one exists; if not, Select 
...
6) Review the settings and click **Save and deploy**, and wait for the deployment build to complete, which will take 
   some time.
![alt text](audio-processing-frontend-deploy-success.png)
